...
from NssMPC.application.neural_network.layers.activation import SigmaReLUTest, UnauthSharkReLUTest
...
# 2. 定义 ReLU 计算函数
def unauth_shark_relu_server(x_shift):
    """服务器端的计算逻辑"""
    with PartyRuntime(server):
        start = time.time()
        relu_layer = UnauthSharkReLUTest()
...
def unauth_shark_relu_client(x_shift):
...
if __name__ == "__main__":
    ...
    UnauthSharkDReLUKey.gen_and_save(X[0])
    server_relu_thread = threading.Thread(target=unauth_shark_relu_server, args=(X[0],))
    client_relu_thread = threading.Thread(target=unauth_shark_relu_client, args=(X[1],))
